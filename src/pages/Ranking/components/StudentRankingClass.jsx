...
export default function StudentRankingClass() {
  ...
  useEffect(() => {
    if (isStudent && user?.className) {
      setClassInput(user.className);
      handleFetch(user.className);
    }
  }, [user, isStudent]);

  // Fetch API ranking theo lớp
  const handleFetch = async (clsInput) => {
    let cls = '';
    if (typeof clsInput === 'string') {
      cls = clsInput;
    } else {
      cls = classInput;
    }
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center gap-2 mb-4">
            ...
            {isStudent && className && <Tag color="blue">Lớp {className}</Tag>}
          </div>
          ...
          {className !== null && (
            <>
              ...
              <div className="w-full overflow-auto">
                <Table dataSource={paginatedData} columns={columns} pagination={false} rowKey="playerId" loading={loading} size="small" bordered scroll={{ x: 'max-content' }} />
              </div>
